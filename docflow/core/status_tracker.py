...
class StatusTracker:
    ...
    def __init__(
        ...
        self.default_branches = set(default_branches or ("main", "master", "develop", "HEAD"))
    ...
    def scan_wip(self) -> Dict[str, Any]:
        ...
        def add_branch(name: str, commit, status: str) -> None:
            if name in self.default_branches or name in seen:
                return
            seen.add(name)
        ...
        for branch in self.app_repo.branches:
            add_branch(branch.name, branch.commit, "in-progress")

        try:
            for remote in self.app_repo.remotes:
                for ref in remote.refs:
                    short = ref.remote_head if hasattr(ref, "remote_head") else ref.name
                    if short in self.default_branches:
                        continue
                    add_branch(short, ref.commit, "remote")
        except Exception:
            pass
